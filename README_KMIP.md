...
| `KMIP_PKCS11_Overview_Deck.pptx` | `generate_overview_deck.js` | 15-slide overview deck: what KMIP is, the design on PKCS#11, the REST-on-KMIP target architecture, the gap analysis and the roadmap. Its gap figures are read from the matrix generator and its roadmap from the delivery plan, so neither can drift |
| `KMIP_PKCS11_Overview_Deck.pdf` | `npm run pdf` | The same deck as a PDF, for reading without PowerPoint |
| `KMIP_PKCS11_REST_KMS_Plan.docx` | `generate_rest_kms_plan.py` | Twenty-six-step plan to a complete KMS with a REST control plane — design, implementation, tests and a gate for each step, closing all 50 open gap-matrix features. Seven steps are blocked on hardware, a product or an external event, and the plan names which |
| `KMIP_PKCS11_REST_KMS_Plan_Deck.pptx` / `.pdf` | `generate_rest_kms_deck.js` | The plan as 81 slides — design impact, proposed solution and test strategy for each of the twenty-six steps |
