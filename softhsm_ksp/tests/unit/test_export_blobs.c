/* test_export_blobs.c — Couverture de P11_ExportRsaPublicKey et P11_ExportEcPublicKey
 * Utilise le mock PKCS#11 pour contrôler les attributs retournés.
 */
...
/* Stubs du contexte p11 pour contourner le singleton */
...
int main(void)
{
    ...
    /* ── Suite 1 : Export RSA clé publique nominale ─────────────────────── */
    TEST_SUITE("P11_ExportRsaPublicKey — cas nominaux");

    /* RSA 2048 bits (modulus=256 octets, exposant=3 octets) */
    ...
    /* Vérifie l'exposant (01 00 01 = 65537) */
    ...
    /* ── Suite 2 : Export RSA avec erreurs d'attribut injectées ─────────── */
    TEST_SUITE("P11_ExportRsaPublicKey — erreurs injectées");

    P11Mock_GetConfig()->rv_GetAttributeValue = CKR_ATTRIBUTE_SENSITIVE;
    ss = P11_ExportRsaPublicKey(0xBEEF, 0x05, &pbBlob, &cbBlob);
    ASSERT_ERR("GetAttributeValue échoue → erreur", ss);
    ASSERT_NULL("pbBlob NULL en cas d'erreur", pbBlob);
    P11Mock_Reset();
    g_testCtx.pFunctionList = P11Mock_GetFunctionList();

    /* ── Suite 3 : Export EC clé publique P-256 nominale ───────────────── */
    TEST_SUITE("P11_ExportEcPublicKey — P-256 cas nominal");
    ...
    /* Vérifie Qx (32 premiers octets après header) */
    BYTE *pbQx = pbBlob + sizeof(BCRYPT_ECCKEY_BLOB);
    ASSERT_EQ("Qx[0] = 0xAA (premier octet fictif)", pbQx[0], 0xAA);

    KSP_Free(pbBlob); pbBlob = NULL; cbBlob = 0;

    /* ── Suite 4 : Export EC P-384 ──────────────────────────────────────── */
    TEST_SUITE("P11_ExportEcPublicKey — P-384");

    static const char oidP384[] = "\x06\x05\x2b\x81\x04\x00\x22";
    /* Point P-384 fictif : DER wrapper + 04 + Qx(48) + Qy(48) = 2 + 97 = 99 */
    static char ecPt384[99];
    ecPt384[0] = 0x04; ecPt384[1] = 0x61; /* DER wrapper */
    ecPt384[2] = 0x04;                     /* non-compressé */
    memset(ecPt384 + 3, 0xBB, 96);         /* Qx + Qy fictifs */
    ...
    ASSERT_EQ("Taille blob P-384 = 8 + 96",
    ...
    /* ── Suite 5 : Export EC erreurs injectées ──────────────────────────── */
    TEST_SUITE("P11_ExportEcPublicKey — erreurs");

    P11Mock_Reset();
    g_testCtx.pFunctionList = P11Mock_GetFunctionList();

    /* Point compressé (commence par 0x02) → rejeté */
    static char badPoint[67];
    badPoint[0] = 0x04; badPoint[1] = 0x41;
    badPoint[2] = 0x02; /* 0x02 = compressé, pas 0x04 */
    P11Mock_GetConfig()->pbEcPoint = badPoint;
    P11Mock_GetConfig()->cbEcPoint = 67;

    ss = P11_ExportEcPublicKey(0xBEEF, 0x05, &pbBlob, &cbBlob);
    ASSERT_ERR("Point EC compressé → NTE_BAD_KEY", ss);
    ASSERT_NULL("pbBlob NULL en cas d'erreur EC", pbBlob);

    /* GetAttributeValue échoue */
    P11Mock_Reset();
    g_testCtx.pFunctionList = P11Mock_GetFunctionList();
    P11Mock_GetConfig()->rv_GetAttributeValue = CKR_OBJECT_HANDLE_INVALID;
    ss = P11_ExportEcPublicKey(0xBEEF, 0x05, &pbBlob, &cbBlob);
    ASSERT_ERR("GetAttributeValue échoue (EC) → erreur", ss);
    ...
    /* FindObjectsInit échoue → CK_INVALID_HANDLE */
    P11Mock_GetConfig()->nKeyObjects   = 1;
    P11Mock_GetConfig()->rv_FindObjectsInit = CKR_DEVICE_ERROR;
    hFound = P11_FindObjectByLabel(0xBEEF, CKO_PRIVATE_KEY, L"TestKey");
    ASSERT_EQ("FindObjectsInit échoue → CK_INVALID_HANDLE",
              hFound, CK_INVALID_HANDLE);

    /* FindObjects échoue → CK_INVALID_HANDLE */
    P11Mock_Reset();
    g_testCtx.pFunctionList = P11Mock_GetFunctionList();
    P11Mock_GetConfig()->nKeyObjects = 1;
    P11Mock_GetConfig()->rv_FindObjects = CKR_DEVICE_ERROR;
    hFound = P11_FindObjectByLabel(0xBEEF, CKO_PRIVATE_KEY, L"TestKey");
    ASSERT_EQ("FindObjects échoue → CK_INVALID_HANDLE",
              hFound, CK_INVALID_HANDLE);
    ...
    rv = P11_GetUlongAttr(0xBEEF, 0x10, CKA_KEY_TYPE, &ulVal);
    ASSERT_NEQ("GetUlongAttr (erreur) → != CKR_OK", rv, (CK_RV)CKR_OK);
