...
five defects that the mock assertions had not: a size query that left an
operation active on a pooled session, an ECDSA path that DER-decoded
signatures PKCS#11 mandates be raw, a certificate left orphaned on the token
when its key was deleted, a symmetric key that could never sign at all, and
AES keys created without the right to wrap. See
